...
class TestSongScrounger(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_create_playlist(self):
        ...
        self.song_scrounger._get_songs = AsyncMock(return_value=["Mock Spotify Track"])

        playlist = await self.song_scrounger.create_playlist("Mock Playlist Name", ["mock song"])
    ...
    async def test_get_songs(self):
        ...
        playlist = await self.song_scrounger._get_songs(["mock song"])

        self.mock_spotify_client.find_track.assert_called_once_with("mock song")

    async def test_get_songs__single_track(self):
        ...
        tracks = await self.song_scrounger._get_songs(mock_song)
    ...
    async def test_find_songs__no_dups(self, mock_read_file_contents):
        tracks = self.song_scrounger.find_songs("mock file path")
    ...
    async def test_get_songs__multiple_tracks(self):
        mock_spotify_track = MagicMock(name="Mock Spotify Track")
        self.mock_spotify_client.configure_mock(**{
            "find_track.return_value": [mock_spotify_track]
        })
        ...
        tracks = await self.song_scrounger._get_songs(mock_tracks)
    ...
    async def _run_integration_test(self, input_file_name, playlist_name):
        ...
        songs = song_scrounger.find_songs(input_file_path)

        await song_scrounger.create_playlist(playlist_name, songs)
